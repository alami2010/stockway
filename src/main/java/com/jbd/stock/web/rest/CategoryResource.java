...
import com.jbd.stock.service.CategoryQueryService;
import com.jbd.stock.service.CategoryService;
import com.jbd.stock.service.criteria.CategoryCriteria;
...
/**
 * REST controller for managing {@link com.jbd.stock.domain.Category}.
 */
@RestController
@RequestMapping("/api")
public class CategoryResource {
    ...
    public ResponseEntity<List<CategoryDTO>> getAllCategoriesFull(
        CategoryCriteria criteria,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get  full Categories by criteria: {}", criteria);
        Page<CategoryDTO> page = categoryQueryService.findByCriteria(criteria, Pageable.unpaged());
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
