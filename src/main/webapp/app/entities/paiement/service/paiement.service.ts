...
type RestOf<T extends IPaiement | NewPaiement> = Omit<T, 'dateCreation'> & {
...
export class PaiementService {
  ...
  protected convertDateFromClient<T extends IPaiement | NewPaiement | PartialUpdatePaiement>(paiement: T): RestOf<T> {
    return {
      ...paiement,
      dateCreation: paiement.dateCreation?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPaiement: RestPaiement): IPaiement {
    return {
      ...restPaiement,
      dateCreation: restPaiement.dateCreation ? dayjs(restPaiement.dateCreation) : undefined,
    };
  }
